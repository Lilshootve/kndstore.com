...
function generateHeader($title = null, $description = null) {
    ...
    // Usar t() si no se proporciona título/descripción
    if ($title === null) {
    ...
    $header = '<!DOCTYPE html>' . "\n";
    $header .= '<html lang="' . htmlspecialchars($current_lang) . '" data-bs-theme="dark">' . "\n";
    $header .= '<head>' . "\n";
...
function generateNavigation() {
    ...
    // Helper para generar URL con cambio de idioma
    $buildLangUrl = function($lang) use ($current_page) {
        $params = $_GET;
        $params['lang'] = $lang;
        $query = http_build_query($params);
        return '/' . $current_page . ($query ? '?' . $query : '?lang=' . $lang);
    };
    ...
    $nav .= '                    <a class="nav-link' . ($current_page == 'index.php' ? ' active' : '') . '" href="/index.php">' . htmlspecialchars(t('nav.home')) . '</a>' . "\n";
    $nav .= '                </li>' . "\n";
    $nav .= '                <li class="nav-item">' . "\n";
    $nav .= '                    <a class="nav-link' . ($current_page == 'products.php' ? ' active' : '') . '" href="/products.php">' . htmlspecialchars(t('nav.catalog')) . '</a>' . "\n";
    $nav .= '                </li>' . "\n";
    $nav .= '                <li class="nav-item">' . "\n";
    $nav .= '                    <a class="nav-link' . ($current_page == 'about.php' ? ' active' : '') . '" href="/about.php">' . htmlspecialchars(t('nav.about')) . '</a>' . "\n";
    $nav .= '                </li>' . "\n";
    $nav .= '                <li class="nav-item">' . "\n";
    $nav .= '                    <a class="nav-link' . ($current_page == 'contact.php' ? ' active' : '') . '" href="/contact.php">' . htmlspecialchars(t('nav.contact')) . '</a>' . "\n";
    $nav .= '                </li>' . "\n";
    $nav .= '                <li class="nav-item">' . "\n";
    $nav .= '                    <a class="nav-link' . ($current_page == 'order.php' ? ' active' : '') . '" href="/order.php">' . "\n";
    $nav .= '                        <i class="fas fa-shopping-cart me-1"></i>' . "\n";
    $nav .= '                        ' . htmlspecialchars(t('nav.order')) . "\n";
    ...
    $nav .= '                        <a href="' . htmlspecialchars($buildLangUrl('es')) . '" class="btn btn-sm ' . ($current_lang === 'es' ? 'btn-neon-primary' : 'btn-outline-light') . '">' . htmlspecialchars(t('nav.language.es')) . '</a>' . "\n";
    $nav .= '                        <a href="' . htmlspecialchars($buildLangUrl('en')) . '" class="btn btn-sm ' . ($current_lang === 'en' ? 'btn-neon-primary' : 'btn-outline-light') . '">' . htmlspecialchars(t('nav.language.en')) . '</a>' . "\n";
    $nav .= '                    </div>' . "\n";
    $nav .= '                </li>' . "\n";
    $nav .= '            </ul>' . "\n";
    $nav .= '        </div>' . "\n";
...
function generateColorPanel() {
    ...
    $panel .= '        <h3><i class="fas fa-magic me-2"></i>' . t_html('color_panel.title') . '</h3>' . "\n";
    $panel .= '    </div>' . "\n";
    $panel .= '    <div class="color-panel-content">' . "\n";
    $panel .= '        <div class="color-theme active" data-theme="galactic-blue">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.galactic_blue') . '</h4>' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.galactic_blue_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '        ' . "\n";
    $panel .= '        <div class="color-theme" data-theme="cyber-green">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.cyber_green') . '</h4>' . "\n";
    $panel .= '            <div class="color-preview">' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.cyber_green_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '        ' . "\n";
    $panel .= '        <div class="color-theme" data-theme="nature-green">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.nature_green') . '</h4>' . "\n";
    $panel .= '            <div class="color-preview">' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.nature_green_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '        ' . "\n";
    $panel .= '        <div class="color-theme" data-theme="fire-red">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.fire_red') . '</h4>' . "\n";
    $panel .= '            <div class="color-preview">' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.fire_red_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '        ' . "\n";
    $panel .= '        <div class="color-theme" data-theme="golden-sun">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.golden_sun') . '</h4>' . "\n";
    $panel .= '            <div class="color-preview">' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.golden_sun_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '        ' . "\n";
    $panel .= '        <div class="color-theme" data-theme="neon-pink">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.neon_pink') . '</h4>' . "\n";
    $panel .= '            <div class="color-preview">' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.neon_pink_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '        ' . "\n";
    $panel .= '        <div class="color-theme" data-theme="ice-blue">' . "\n";
    $panel .= '            <h4>' . t_html('color_panel.theme.ice_blue') . '</h4>' . "\n";
    $panel .= '            <div class="color-preview">' . "\n";
    ...
    $panel .= '            </div>' . "\n";
    $panel .= '            <p>' . t_html('color_panel.theme.ice_blue_desc') . '</p>' . "\n";
    $panel .= '        </div>' . "\n";
    $panel .= '    </div>' . "\n";
    $panel .= '</div>' . "\n";
