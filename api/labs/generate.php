...
try {
    ...
    $stmt = $pdo->prepare(
        "INSERT INTO knd_labs_jobs (user_id, tool, prompt, negative_prompt, comfy_prompt_id, status)
         VALUES (?, ?, ?, ?, ?, 'pending')"
    );
    if (!$stmt || !$stmt->execute([$userId, $tool, $prompt, $negativePrompt, $promptId])) {
        json_error('DB_ERROR', 'Could not create job. Run sql/knd_labs_jobs.sql if needed.', 500);
    }
